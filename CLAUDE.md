...
- The require-cache trap when iterating on the multi-file engine
...
- For the engine's multi-file demo, the MCP's `file` parameter shows stale geometry after sub-module edits. Use inline `code:` with `delete require.cache[...]` (pattern in AGENTS.md → "The require-cache trap").
